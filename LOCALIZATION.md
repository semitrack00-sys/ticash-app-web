...
`countryFlag(code)` remains available for text/search compatibility, while the visible country picker uses locally committed SVG assets under `/flags/<iso>.svg`. This avoids Windows/Chrome rendering regional-indicator emoji as two letters. The picker does not use an external flag CDN or runtime network dependency; country names, ISO codes and calling codes remain visible fallbacks.
